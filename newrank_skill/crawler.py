...
class NewRankCrawler:
    ...
    def __init__(self, output_dir: str = "output/articles", db_path: str = "crawled_memory.db"):
    ...
    def _save_results(self):
        """Save unique results to individual JSON files and update memory bank."""
        import os
        import re
        ...
        for item in new_items:
            try:
                ...
                # Truncate if too long (Windows max path is 260, filename max 255)
                safe_title = safe_title[:50] 
                filename = f"{item.pub_time.replace(':', '').replace(' ', '_')}_{safe_title}.json"
                ...
                logger.error(f"Failed to save article '{item.title}': {e}")
                
        logger.info(f"Saved {saved_count} new items to {self.output_dir}")
